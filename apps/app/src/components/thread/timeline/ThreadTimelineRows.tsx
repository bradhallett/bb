...
export interface ThreadTimelineRowsProps {
  /** Enable the opt-in normal-flow row windowing treatment. */
...
function TimelineRowItemWrapper({
  ...
  return (
    <div
      ...
      style={
        windowedState.placeholderStyle ??
        (isTopLevel && !windowedState.windowingEnabled
...
function TimelineRowsList({
  ...
  const standaloneMeasurementsRef = useRef(new Map<string, number>());
  const measurements =
    inheritedMeasurements ?? standaloneMeasurementsRef.current;
  ...
  const alwaysMountedKeys = useMemo(() => {
    ...
    return keys;
  }, [items, rows, stableSearchExpandedRowIds]);
            ...
            if (item.kind === "unread-divider") {
              return (
                <div
                  ...
                  style={windowedState.placeholderStyle}
                >
...
function ThreadTimelineRowsForTimelineView(props: ThreadTimelineRowsProps) {
  ...
  const windowingMeasurementsRef = useRef(new Map<string, number>());
  const rows = useMemo(
              ...
              >
                ...
                >
                  <TimelineWindowingMeasurementsContext.Provider
                    value={windowingMeasurementsRef.current}
                  >
