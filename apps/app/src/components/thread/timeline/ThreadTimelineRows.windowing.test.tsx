...
beforeEach(() => {
  ...
  vi.spyOn(HTMLElement.prototype, "clientHeight", "get").mockImplementation(
    function (this: HTMLElement) {
      return this.hasAttribute("data-detail-scroll-area") ? 500 : 0;
    },
  );
...
describe("ThreadTimelineRows windowing experiment", () => {
  ...
  it("windows the rows inside a large expanded detail", () => {
    ...
    expect(detailScroll?.clientHeight).toBe(500);
    expect(
      Array.from(wrappers ?? [])
        .slice(0, 5)
        .map((wrapper) => [
          wrapper.getAttribute("data-timeline-row-id"),
    ...
    expect(
      nestedList?.querySelectorAll(
        ':scope > [data-timeline-windowed-realized="true"]',
      ).length,
    ).toBeLessThan(10);
    expect(
      nestedList?.querySelectorAll(
        ':scope > [data-timeline-windowed-realized="false"]',
      ).length,
    ).toBeGreaterThan(20);
  });
